...
- **Optional AI helper** – bring your own key (OpenAI, Claude, Gemini or DeepSeek) and let the model write a part's behavior note or a screen's description. Each rewrite can be undone. The key stays in your browser and the request goes straight to the provider; there is no server in between.
- **Export & Architecture Diagrams** – copy the AI prompt (edit it by hand first if you like), export clean YAML code formatted for **Microsoft Power Automate** & Power Apps workflows (or full declarative UI specifications), render and export live **Mermaid.js Flowcharts** for Jira/Confluence/GitHub, download `.yaml` and `.mmd` files directly, or save screens as PNG.
- **Corporate Template Packs** – 1-click starter templates for enterprise apps: Executive KPI Dashboard (Desktop & Mobile), Multi-step Expense Reimbursement Flow, and IT Service Desk Catalog.
...
Marsh-Stitch is free and MIT-licensed, and stays that way. If it saves you time, you can [sponsor the work on GitHub](https://github.com/sponsors/mannie09); it pays for the hours that go into new parts, templates, and ongoing development. No feature is behind sponsorship.
