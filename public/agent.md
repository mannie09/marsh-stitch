# Marsh-Stitch: sketches from an AI agent (beta)

Marsh-Stitch (https://mannie09.github.io/marsh-stitch/) is a browser editor for Material 3 Expressive screens. A design is one JSON document. You, the agent, write that document and hand it back; the person opens it on their canvas, refines it, and turns it into a prompt for a coding tool.
...
console.log("https://mannie09.github.io/marsh-stitch/#docz=" + deflateRawSync(json).toString("base64url"));
```
...
print("https://mannie09.github.io/marsh-stitch/#docz=" + base64.urlsafe_b64encode(raw).decode().rstrip("="))
```

The link is long (a few thousand characters for a few screens). That is expected; it carries the whole design and nothing is stored anywhere. If you fetched this guide from a different address than `https://mannie09.github.io/marsh-stitch/agent.md`, build the link on that address instead (the app lives next to its guide).
